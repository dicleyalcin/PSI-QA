...
for i in class_labels:
	cpath=Path(cwd+"/ROI_Groups"+"/"+str(i))
	#print(cpath)
			...
			if dat2 in onlyfiles[annot]:
				...
				if dat32 in onlyfiles[annot]:
					...
					if platform.system()=='Windows':
						...
						#print(wpmeta3)
						cdest = str(Path(cwd + '\ROI_Groups\\' + str(metasource[entry][5])))
						#print(cdest)
						shutil.copy("{0}".format(wpmeta3), "{0}".format(cdest))
					else:
						command = "cp %s/ROIs_raw_results_trimmed/%s %s/%s/" % (cwd, U, cwd+"/ROI_Groups",str(metasource[entry][5]))
						os.system(command)
					#print(command)
					#break
					#os.system(command)
