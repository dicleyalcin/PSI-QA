...
# Windows-specific DLL issue (a possible workaround). Change the name provided in the openslide_path to the appropriate
# name stated in the directory, depending on the version installed.
...
#def get_annotation_bounds(xml_path):
    ...
    #print("./Annotation[@Name='" + "Annotation " + str(8) + "']/Metadata")   
    #print(root.tag)
    #print(root.attrib)  
    bounds = []
    masks = []
    ...
    for i, child in enumerate(root): # each annotation
        ...
        color = d.get('Color')
        colors.append(color)
        #convertedColor = colconvert(color)
        # {'Visible': '-1', 'Selected': '0', 'FontItalic': '0', 
        # 'FontUnderline': '0', 'GUID': '67a6a154-bb1a-4b0f-b716-51b3873ea44f', 
        # 'Width': '10', 'Color': '4294901760', 'FontBold': '0', 'Measurement': '1', 
        # 'Subtype': '1', 'FontFamily': 'Arial', 'DetailVisible': '0', 'Type': 'Pointn', 'FontSize': '12'}
        #print(d)
        #print(d.get('Color')) # Get unique list of colors for regions
        origx = []
        origy = []
        x=[]
        y=[]
        #if d.get('Color') == color: # Region: 
            #print("found the region")
            #print(d)
            #break
            #annots_4294902015.append(child)
            #AllAttributes = []
        data_of_interest = []
        for grandchild in child:
            #g = grandchild.attrib            
            #AllAttributes.append(g)
            if grandchild.tag=="S":
        ...
        #x = side1+
        #x = list(range(data_of_interest[0],data_of_interest[0]+data_of_interest[3],20))
        #y = list(range(data_of_interest[1],data_of_interest[1]+data_of_interest[2],20))
        #print(y)
        #print(x[0],y[0])
        #break
        ...
        ask = np.zeros([cx, cy], dtype=np.int8)
